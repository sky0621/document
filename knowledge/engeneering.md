...
「振り返り」により失敗に対する打ち手を考える。

どこに向かって振り返るかを定めるために「インセプション・デッキ」を用いる。
...
「個人の総和が組織の能力にならない」…個人間が100%の情報伝達と保持できない限り、人数が増えるほど「調整コスト」は増す。

「コンウェイの法則」…システムを設計する組織は、その構造をそっくりまねた構造の設計を生み出す。（コミュニケーションコストを避けるため、結果そうなる）

「ソフトウェア開発上の問題の多くは、技術的というより社会学的なものである」…トム・デマルコの『ピープルウェア』

### 「権限」の委譲

「権限」…会社の資産やリソースに対して何をしてよいかという自由度。

「説明責任」…与えられた権限に基づき、何を行い、どのような結果になったかを権限付与者に報告する責務。

「責任」と「権限」は一致している必要がある。

誰にどのような権限を与えるか、各人が現状、どのような権限があると考えているかを知る方法として「デリゲーションポーカー」がある。

（上司と部下の間で心理的安全性を維持しながら、テーマ毎にどの権限レベルにあるかを話し合う）

権限が決まったら、それをコンセンサスボードにて可視化する。

### 技術的負債

ウォード・カニングが提唱。

「最初のコードを出荷することは、借金をしに行くのと同じである。

　小さな負債は、代価を得て即座に書き直す機会を得るまでの開発を加速する。
 
　危険なのは、借金が返済されなかった場合である。

　品質のよくないコードを使い続けることは、借金の利息としてとらえることができる。
